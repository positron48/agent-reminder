...
> **macOS (без Apple-подписи):** при первом запуске macOS покажет предупреждение безопасности. **Правый клик по приложению → Открыть**, или после неудачной попытки: **Системные настройки → Конфиденциальность и безопасность → Всё равно открыть**. Для open-source-сборки это нормально.
